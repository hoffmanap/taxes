...
6. **Quantified:** removing just the single-family-driven gap in El Paso's home value (holding income's effect and its actual tax bill fixed) would cut its effective rate from 2.17% to 1.78% — an 18% reduction, closing roughly a third of the entire gap between El Paso and the average large city's rate. Two independent coefficients both point the same direction: each percentage point of housing stock shifted away from single-family is associated with ~0.8% higher home value and ~3% higher per-capita service spending. In this data, lowering the tax rate and raising service levels are not a trade-off — they're the same lever (§06).
7. **Does the type of multifamily matter?** Tested using the ACS's finer structure-size breakdown (duplex through 50+ units). For service spending, missing-middle housing (2-9 units) and large multifamily (10+ units) have statistically indistinguishable effects (p=0.97) — one point of missing-middle infill does about as much as one point of large-apartment development. For home value, large multifamily's effect is individually significant (p<0.001) while missing-middle's is only borderline (p=0.081), but a direct test of whether the two differ from each other isn't significant either (p=0.38) — the 47-city sample isn't quite large enough to confirm structure size matters for the tax-rate side, though the point estimates hint that direction. Practically: nothing in the data rules out missing-middle infill as a productive lever, even though it's likely an easier political and physical lift than large apartment construction (§06).
